...
let style = "position:fixed;display:block;width:100px;height:100px;right:20px;bottom:35px;background-color:transparent;z-index:99;opacity:1"
...
function initJoystick() {
  ...
  if(useragent=="mobile"){
    ...
    document.getElementById('np').style.right = '165px';
    //document.querySelector('#np').style.bottom = '20px';
  ...
  const language = localStorage.getItem('languageselection') // Assuming language is stored in localStorage or default to English
  if (language === 'hindi') {
      p.innerHTML = "पकड़ो और खींचो"; // Hindi translation for "Hold and drag"
  } else {
      p.innerHTML = "Hold and drag"; // English
  }
  d.appendChild(p);
  // create image element
  var img = document.createElement("img");
  img.setAttribute("src", "assets/icons/joystickimg.png"); // Replace with the path to your image
  img.setAttribute("style", "width: 80px; height: 80px; margin: -10px auto; display: block;pointer-events: none;");
  img.setAttribute('concontextmenu','return false');
  d.appendChild(img);
